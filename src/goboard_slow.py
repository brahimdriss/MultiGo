...
class Move:
    # Following American Go Association (AGA), the term  'move' is used to mean the 3 possible
    # actions (play, pass, resign) whereas a 'play' refers to playing a stone.
...
class GoString:
    # We will use strings of stones that are a group of connected stones in order to reduce
    # computational complexity in board checking
    def __init__(self, color, stones, liberties):
        # Go strings are chain of connected stones of the same color
        self.color = color
...
class Board:
    ...
    def place_stone(self, player, point):
        ...
        for (
            same_color_string
        ) in adjacent_same_color:  # Merge any adjacent strings of the same color
        ...
        for (
            ...
            adjacent_opposite_color
        ):  # remove opposite color strings if they have 0 liberties
...
class GameState:
    ...
    def apply_move(self, move):
        """
        Returns a new GameState after playing
        """
        if move.is_play:
    ...
    def winner(self):
        # Keep this only for mcts, remove later
        # if not self.is_over():
        # return None
        if self.last_move.is_resign:
